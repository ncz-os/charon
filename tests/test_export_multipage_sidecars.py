...
def _mv_entry(mv_id: str, record_id: str, version_num: int, *, parent: Optional[str] = None,
              content: Optional[str] = None, branch: str = "main"):
    entry: Dict[str, Any] = {
        "id": mv_id,
        "record_id": record_id,
...
def _cm_entry(record_id: str, engine_id: str = "apollo", engine_version: str = "1.0",
...
def test_cli_aggregates_per_memory_sidecars_across_two_pages():
    ...
    assert record_ids == {"mem_A", "mem_B"}, (
        f"expected both page-1 and page-2 records in the final envelope; "
...
def test_cli_memory_versions_are_ordered_by_record_then_version_num():
    ...
    for record_id, versions in by_record.items():
        ...
        assert version_nums == sorted(version_nums), (
            f"versions for {record_id} are not monotonically ordered: "
            f"{version_nums}"
        )
    ...
    assert actual_order == expected_order, (
        f"memory_versions ordering mismatch: got {actual_order}, "
...
def test_cli_dedupes_identical_overlapping_sidecar_rows():
    ...
    kg_ids = [kg["id"] for kg in result.get("kg_triples", [])]
    assert kg_ids.count("kg_overlap") == 1, (
        f"overlapping kg_triple was double-counted: got {kg_ids}"
    )
    assert "kg_B1" in kg_ids, "page-2's distinct kg_triple was lost"

    mv_ids = [mv["id"] for mv in result.get("memory_versions", [])]
    assert mv_ids.count("ver_A_v1") == 1, (
        f"overlapping memory_version was double-counted: got {mv_ids}"
    )
    assert "ver_B_v1" in mv_ids, "page-2's distinct memory_version was lost"

    cm_record_engine = [
        (cm["record_id"], cm["engine_id"], cm.get("engine_version"))
        for cm in result.get("compression_manifest", [])
    ]
    assert cm_record_engine.count(("mem_A", "apollo", "1.0")) == 1, (
        f"overlapping compression_manifest was double-counted: "
        f"got {cm_record_engine}"
    )
    assert ("mem_B", "apollo", "1.0") in cm_record_engine


def test_cli_warns_on_divergent_overlapping_sidecar_rows(capsys):
    """If two pages emit the SAME identity with DIFFERENT content, the
    CLI must NOT silently drop — it must keep the first observation and
    emit a stderr warning so an operator notices the server-side
    inconsistency."""
    ...
    with _patch_urlopen([page1, page2, SHORT_PAGE]) as captured_err:
        from mnemos.tools.memory_export import _fetch_export

        result = _fetch_export(
            endpoint="http://testserver",
            api_key=None,
            category=None,
            limit=1,
    ...
    assert "WARNING" in err_output, (
        f"expected stderr warning on divergent duplicate; got: {err_output!r}"
    )
    assert "kg_triples" in err_output, (
        f"expected warning to name the surface; got: {err_output!r}"
    )
    # First observation wins.
    surviving = [kg for kg in result.get("kg_triples", []) if kg["id"] == "kg_dup"]
    assert len(surviving) == 1
    assert surviving[0]["subject_literal"] == "Paris", (
        f"first observation should win on identity collision; got "
        f"subject={surviving[0].get('subject_literal')!r}"
    )
...
def test_cli_keeps_deletion_log_separate_from_per_memory_sidecars():
    ...
    page1 = _envelope_page(
        records=[_memory_record("mem_A")],
        kg_triples=_PAGE_1_KG_FOR_DL if False else PAGE_1_KG,
        memory_versions=PAGE_1_MV,
        compression_manifest=PAGE_1_CM,
        deletion_log=[],
    )
...
class _Conn:
    ...
    def __init__(self, *, allowlist_memory_ids: Optional[List[str]] = None) -> None:
    ...
    async def fetch(self, sql: str, *args):
        ...
        if "FROM memories WHERE id = ANY" in sql:
            return [
                {"id": mid, "owner_id": "alice", "namespace": "alice-ns"}
                for mid in self._allowlist_memory_ids
            ]
...
def _install_mock_pool(monkeypatch, conn):
    """Install ``conn`` as the pool acquisition target."""
...
def test_cli_two_page_export_envelope_round_trips_through_import():
    ...
    assert record_ids == {"mem_A", "mem_B"}, (
        f"CLI export fixture broken: expected both mem_A and mem_B in "
        f"the final envelope, got {sorted(record_ids)}"
    )
    ...
    mem_b_versions = [
        mv for mv in envelope.get("memory_versions", []) if mv["record_id"] == "mem_B"
    ]
    ...
    inserted_record_ids = {
        # record_id is positional arg #0 in the INSERT (see
        # repo.insert_memory_versions — first positional is memory_id
        # / record_id). Search the args tuple for any string that looks
        # like our fixture ids.
        arg for e in mv_inserts for arg in e[1] if isinstance(arg, str)
    }
...
def test_cli_export_pagination_loop_emits_one_urlopen_per_page():
    ...
    assert call_count["n"] == 3, (
        f"expected 3 urlopen calls (2 record pages + 1 short page); "
        f"got {call_count['n']}"
    )
...
def test_cli_no_sidecars_means_no_sidecars_in_final_envelope():
    ...
    result = _run_cli_export(
        pages=[page1, page2, SHORT_PAGE], include_sidecars=False
    )
